            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode >= 400) {
                        reject(new Error(`Kie API error ${res.statusCode}: ${JSON.stringify(parsed)}`));
                    } else {
                        resolve(parsed);
                    }
                } catch (e) {
...
async function createNanoBananaTask(input) {
    ...
    console.log('[Kie] Creating Nano Banana 2 task...');
...
async function getTaskDetails(taskId) {
    ...
    const result = await apiRequest('/recordInfo', 'POST', { taskId });
    return result;
}
